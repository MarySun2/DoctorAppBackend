...
namespace BLL.Servicios
{
    public class HistoriaClinicaServicio : IHistoriaClinicaServicio
    {
        ...
        public async Task Actualizar(HistoriaClinicaDto modelDto)
        {
            try
            {
                // Buscar la historia clínica por el ID del paciente
                var historiaClinicaDb = await _unidadTrabajo.HistoriaClinica.ObtenerPrimero(
                    h => h.PacienteId == modelDto.PacienteId,
                    incluirPropiedades: "Paciente");
                if (historiaClinicaDb == null)
                    throw new TaskCanceledException("La historia clínica no existe");

                // Actualizar datos del paciente
                historiaClinicaDb.Paciente.Apellidos = modelDto.Apellidos;
                historiaClinicaDb.Paciente.Nombres = modelDto.Nombres;
                historiaClinicaDb.Paciente.Direccion = modelDto.Direccion;
                historiaClinicaDb.Paciente.Telefono = modelDto.Telefono;
                historiaClinicaDb.Paciente.Genero = modelDto.Genero;
                historiaClinicaDb.Paciente.Estado = modelDto.Estado == 1 ? true : false;
                historiaClinicaDb.Paciente.FechaActualizacion = DateTime.Now;

                _unidadTrabajo.Paciente.Actualizar(historiaClinicaDb.Paciente);

                // Actualizar antecedentes (observaciones)
                // Eliminar los antecedentes existentes
                var antecedentesExistentes = await _unidadTrabajo.Antecedente.ObtenerTodos(
                    a => a.HistoriaClinicaId == historiaClinicaDb.Id);
                foreach (var ant in antecedentesExistentes)
                {
                    _unidadTrabajo.Antecedente.Remove(ant);
                }

                // Agregar los nuevos antecedentes
                foreach (var observacion in modelDto.Observaciones)
                {
                    var nuevoAntecedente = new Antecedente
                    {
                        HistoriaClinicaId = historiaClinicaDb.Id,
                        Observacion = observacion,
                        FechaCreacion = DateTime.Now,
                        FechaActualizacion = DateTime.Now
                    };
                    await _unidadTrabajo.Antecedente.Agregar(nuevoAntecedente);
                }
        ...
        public async Task Remover(int pacienteId)
        {
            try
            {
                // Buscar la historia clínica por el ID del paciente
                ...
                var antecedentes = await _unidadTrabajo.Antecedente.ObtenerTodos(
                    a => a.HistoriaClinicaId == historiaClinicaDb.Id);
                foreach (var ant in antecedentes)
                {
                    _unidadTrabajo.Antecedente.Remove(ant);
                }

                // Eliminar historia clínica
                _unidadTrabajo.HistoriaClinica.Remove(historiaClinicaDb);

                // Eliminar paciente
                _unidadTrabajo.Paciente.Remove(historiaClinicaDb.Paciente);
